...
extension HFHomeModel {
    ...
    func parseToListOfHFHomeModelList(data jsonData: Array<Dictionary<String, Any>>) throws -> Array<HFHomeModel> {
        ...
        for dataObject in jsonData {
            ...
            guard let calories = dataObject["calories"] as? String else {
                throw HSParseDataError.invalidVariableMapping("calories")
            }
            
            guard let carbos = dataObject["carbos"] as? String else {
                throw HSParseDataError.invalidVariableMapping("carbos")
            }
            
            guard let card = dataObject["card"] as? String else {
                throw HSParseDataError.invalidVariableMapping("card")
            }
            
            guard let country = dataObject["country"] as? String else {
                throw HSParseDataError.invalidVariableMapping("country")
            }
            
            guard let deliverable_ingredients = dataObject["deliverable_ingredients"] as? Array<String> else {
                throw HSParseDataError.invalidVariableMapping("deliverable_ingredients")
            }
            
            guard let descriptionData = dataObject["description"] as? String else {
                throw HSParseDataError.invalidVariableMapping("description")
            }
            
            guard let difficulty = dataObject["difficulty"] as? Int else {
                throw HSParseDataError.invalidVariableMapping("difficulty")
            }
            
            guard let fats = dataObject["fats"] as? String else {
                throw HSParseDataError.invalidVariableMapping("fats")
            }
            
            guard let favorites = dataObject["favorites"] as? Int else {
                throw HSParseDataError.invalidVariableMapping("favorites")
            }
            
            guard let fibers = dataObject["fibers"] as? String else {
                throw HSParseDataError.invalidVariableMapping("fibers")
            }
            
            guard let headline = dataObject["headline"] as? String else {
                throw HSParseDataError.invalidVariableMapping("headline")
            }
            
            guard let highlighted = dataObject["highlighted"] as? String else {
                throw HSParseDataError.invalidVariableMapping("highlighted")
            }
            
            guard let idData = dataObject["id"] as? String else {
                throw HSParseDataError.invalidVariableMapping("id")
            }
            
            guard let image = dataObject["image"] as? String else {
                throw HSParseDataError.invalidVariableMapping("image")
            }
            
            guard let incompatibilities = dataObject["incompatibilities"] as? String else {
                throw HSParseDataError.invalidVariableMapping("incompatibilities")
            }

            guard let ingredients = dataObject["ingredients"] as? Array<String> else {
                throw HSParseDataError.invalidVariableMapping("ingredients")
            }

            guard let keywords = dataObject["keywords"] as? Array<String> else {
                throw HSParseDataError.invalidVariableMapping("keywords")
            }
